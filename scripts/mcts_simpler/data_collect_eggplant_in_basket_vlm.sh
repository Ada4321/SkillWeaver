#!/bin/bash
#SBATCH --job-name=mcts_simpler_eggplant_in_basket_vlm
...
# =========================
# simpler / BridgeData WidowX SINK task: put the eggplant in the yellow basket.
...
#   bash data_collect_eggplant_in_basket_vlm.sh 1,3,10-12
if [[ "$#" -ge 1 ]] && [[ "$1" != *=* ]]; then
  SCENE_IDS="$1"
  shift
fi
...
# NOTE (env overrides): do NOT pass dome_light / floor_z / camera overrides
# here — conf/env/simpler_sink.yaml PINS the bridge sink camera, dome light
# (250) and table_z for background-overlay compositing; any override breaks the
# registration against the sink photo. front_camera_count stays at the default
# 1 (the camera is pinned, extra views would be identical).
