#!/bin/bash
#SBATCH --job-name=mcts_simpler_stack_green_on_yellow_vlm
...
# =========================
# simpler / BridgeData WidowX table task: stack the green block on the yellow block.
...
# task_ids=[0].
TASK_NAME="stack_green_on_yellow"
...
#   bash data_collect_stack_green_on_yellow_vlm.sh 1,3,10-12
if [[ "$#" -ge 1 ]] && [[ "$1" != *=* ]]; then
  SCENE_IDS="$1"
  shift
fi
